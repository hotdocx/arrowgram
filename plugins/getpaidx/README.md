...
The plugin exposes an inline-capable professional Discover flow plus catalog-backed API discovery and curated workflows for posts, public GitHub repository workspaces, conference peer-review setup and full review cycles, Live Room configuration and occurrences, participant moderation, persistent meeting chat, fixed-price Live Offers, policy-checked links to authenticated UI, Arrowgram workspaces, safe source edits, diffs, builds, snapshots, artifact-site publishing, and workspace closure. Discover returns one minimized card at a time; Pass/Save stay private and Interested requires an explicitly confirmed reply. The repository flow lists built-in pools and current-user billing options, creates a DRAFT, configures runtime/access/billing, starts an edit session, performs a bounded depth-one public-GitHub clone, saves a snapshot, and returns a token-free preview handoff. Use workspace automation for bounded project commands; never seek controller credentials or a raw shell endpoint. Catalog workflows also cover general user-asset upload; binary signed-URL upload happens in the workspace/CLI rather than through the JSON-only raw MCP caller.
